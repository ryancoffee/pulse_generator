...
class PulseFreq 
{
	...
	public:
		...
		PulseFreq & modAmp(std::vector<float> & x)
		{
			if (intime){
				std::cerr << "whoops, trying to add phase in the time domain\n" << std::flush;
				return *this;
		...
		PulseFreq & modulateamp_time(const std::vector<double> & modulation);
		PulseFreq & modulatephase_time(const std::vector<double> & modulation);
